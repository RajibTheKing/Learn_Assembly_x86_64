...
public:
    ...
    using A = const char*; /// TODO should be removed, because each time a comparison is made \see std::strlen() will be called, which most likely eats up all the benefit, therefore its much better to transform it into \see std::string_view and then possibly still do an asymmetric key type lookup
    using B = const std::string&;
    using C = const std::string_view&;
  //using D = const std::pmr::string&;
...
//using LessCompareCaseInsensitive = LessCompareBase<&strnicmp>;
